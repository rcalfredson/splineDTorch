...
JPG_X, AVI_X, AD_AVI_X, TXT_X = (
    re.compile(p, re.IGNORECASE) for p in (r"\.jpg$", r"\.avi$", r"AD\.avi$", r"\.txt$")
)
...
def isClose(v1, v2, atol=1e-08):
    def len1(v):
        return 1 if np.isscalar(v) else len(v)
...
def noneadd(a, val):
    return a if a is None else a + val


# returns a or, iff a is None, val (default: NaN)
def none2val(a, val=np.nan):
    return val if a is None else a
...
def _prefIdxTest():
    ts = [(0, 10, -1), ([10, 15, 5], [0, 5, 4], [1.0, 0.5, np.nan])]
    for a, b, pi in ts:
        test(prefIdx, [a, b], pi)
    ...
    def rate(self):
        ...
        if self.lts is not None:
            ...
            if self.useBuffer:
                ...
                self.dt = (
                    dt if self.dt is None else (1 - self.alp) * self.dt + self.alp * dt
                )
                avgRt = 1 / self.dt
        self.lts = ts
        return avgRt
...
class Overrider:
    ...
    def report(self):
        if self.ovs:
            print(
                "overrode options:\n  %s\n"
                % "\n  ".join('"%s" to %s (from %s)' % e for e in self.ovs)
            )
...
def time2str(secs=None, format="%Y-%m-%d %H:%M:%S", utc=False):
    if secs is None:
        secs = time.time()
    return time.strftime(format, time.gmtime(secs) if utc else time.localtime(secs))
...
def s2time(secs):
    return time2str(secs, "%H:%M:%S", utc=True)


# converts the given frame index to HH:MM:SS
def frame2time(fi, fps):
    return s2time(fi / fps)
...
def _parseWeightsTest():
    ts = [(["1,3,0"], [0.25, 0.75, 0.0]), (["1.8,.2", 2], [0.9, 0.1])]
    for args, r in ts:
...
def join(withStr, l, lim=None, p=None, end=False):
    ...
    return withStr.join(ff % e if ff and isinstance(e, float) else str(e) for e in l)


# joins the given list using "," and "and"
def commaAndJoin(l):
    return join(" and ", l) if len(l) < 3 else join(", ", l[:-1]) + ", and %s" % l[-1]
...
def toNumeric(s, noFloat=False):
    if isinstance(s, str):
        if DIGITS_ONLY.match(s):
            return int(s)
        elif not noFloat and SIMPLE_FLOAT.match(s):
            return float(s)
    return s
...
def toInt(s):
    return toNumeric(s, noFloat=True)
...
# tupleOp() add
def tupleAdd(t1, t2):
    return tupleOp(operator.add, t1, t2)


# tupleOp() subtract
def tupleSub(t1, t2):
    return tupleOp(operator.sub, t1, t2)
...
def _matchArraysTest():
    ts = [
        (([1, 0, 3], [1, 3]), ([True, False, True], True)),
...
def angleDiff(theta, theta2=None, absVal=False):
    result = (
        normAngles(np.diff(theta), False)
        if theta2 == None
        else normAngles(theta - theta2, False)
    )
...
def normAngles(a, useRadians=True):
    return (
        np.mod(a + np.pi, 2 * np.pi) - np.pi
        if useRadians
        else np.mod(a + 180, 360) - 180
    )
...
def rdp(xy, epsilon, useRdpPkg=False):
    ...
    if useRdpPkg:
        ...
        if len(xy) == 1:
            ...
            def getIdxs(xyi):
                return (fctr * xyi[:, 2] + 0.5).astype(int)

            sxy = np.array(sg.LineString(xy).simplify(epsilon, preserve_topology=False))
            si = getIdxs(sxy)
            if _TEST_RDP:
            ...
            if p <= 0.0001
                ...
                if p <= 0.001
                    ...
                    if p <= 0.01
                        ...
                        "*"
                        if p <= 0.05
                        else "ns" + (" (p=%.2f)" % p if nsWithP else "")
                    )
                )
            )
        )
...
def readCsv(fn, toInt=True, nCols=None):
    ...
    with open(fn) as f:
        dt = list(
            csv.reader(
                row for row in (row.partition("#")[0].strip() for row in f) if row
            )
        )
        if nCols:
            for row in dt:
...
def fileList(fn, op=None, tp="videos", pattern=AD_AVI_X):
    ...
    if isd or _GLOB_CHARS.search(fn):
        fns = [
            (os.path.join(fn, f) if isd else f)
...
def fill_in_blanks(img, bg_color):
    img[np.where(np.all(img == (0, 0, 0), axis=-1))] = bg_color
    return img


def get_border_vals(arr):
    return set(
        np.concatenate(
            (
                arr[[0, arr.shape[0] - 1], :].ravel(),
                arr[1:-1, [0, arr.shape[1] - 1]].ravel(),
            )
        )
    )
...
def astype(img, dtype, rint=False):
    return (
        ...
        else (np.rint(img) if rint else img).astype(dtype)
    )
...
def extendImg(img, trbl, color=255):
    h, w = img.shape[:2]
    x, y = trbl[3], trbl[0]
    imgE = getImg(y + h + trbl[2], x + w + trbl[1], nCh=numChannels(img), color=color)
...
def combineImgs(
    ...
    if resizeFctr is not None:
        imgs = [cv2.resize(img, (0, 0), fx=resizeFctr, fy=resizeFctr) for img in imgs]
    ...
    wA = (
        max(w, max(textSize(hdr, style)[0] for hdr in hdrs)) if adjustHS and hdrs else w
    )
    ...
    for i, img in enumerate(imgs):
        r, c = divmod(i, nc)
        xL, yL = int(d / 2 + c * (wA + d)), int(
            d / 2 + hdrLH + hdrLD + hdrH + hd + r * (h + d + hdrH + hd)
        )
        if img is not None:
            h1, w1 = img.shape[:2]
            imgL[yL : yL + h1, xL : xL + w1] = (
                img if img.ndim == ndL else img[..., None]
            )
        if hdrs and hdrs[i]:
...
def mplColormap(cmap=cv2.COLORMAP_JET):
    ...
    return mpl.colors.ListedColormap(cols[:, 0, ::-1] / 255.0)
...
def textStyle(size=0.9, color=COL_BK):
        ...
        for i, cs1 in enumerate(cs):
            if c < len(cs1):
                cv2.putText(
                    img,
                    cs1[c],
                    intR(tupleAdd(pos, (0, i * wh[1] * lnHeightMult))),
                    *style
                )
...
def pilImage(img):
    return Image.fromarray(
        cv2.cvtColor(img, cv2.COLOR_BGR2RGB) if numChannels(img) > 1 else img
    )
...
def v4l2Control(dev, ctl=None, defaultOtherCtls=False):
    ...
    cmd.extend(
        ["--list-ctrls"]
        if getDflts
        else (["-C", ctl] if getCtl else ["-c", ",".join("%s=%s" % cv for cv in ctl)])
    )
...
def v4l2BusInfo(dev=None):
    ...
    out = executeOutput(cmd)
    return (
        dict((bi, int(d)) for bi, d in re.findall(_GET_BUS_INFOS, out))
        if all
        else firstGroup(_GET_BUS_INFO, out)
    )
